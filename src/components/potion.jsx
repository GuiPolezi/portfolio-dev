...
const potionData = [
  { id: 'health', label: 'Health', icon: '#potion', color: 'red', support: 'darkred' },
  { id: 'mana', label: 'Mana', icon: '#potion', color: 'blue', support: 'darkblue' },
  { id: 'stamina', label: 'Stamina', icon: '#potion', color: 'yellow', support: 'orange' },
  { id: 'strength', label: 'Strength', icon: '#enhancement', color: 'purple', support: 'indigo' },
  { id: 'fortify', label: 'Fortify', icon: '#enhancement', color: 'lime', support: 'green' },
...
export default function PotionSelector() {
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      // Se o clique ocorrer fora da div que contém o ref (nosso componente) e ele estiver aberto
    ...
    // Limpeza: remove o ouvinte quando o componente for desmontado para evitar vazamento de memória
    return () => {
  ...
  return (
    <>
      ...
      <div className="potion-selector-wrapper" ref={wrapperRef}>
        {/* Botão Central (Gatilho) */}
        <button
          ...
          {potionData.map((potion, i) => (
            <button
              ...
              style={{
                ...
                '--support-color': potion.support,
              }}
            >
              <svg className="option-icon" aria-hidden="true">
                <use href={potion.icon} />
              </svg>
              <span style={{color: '#606c38'}} ref={(el) => (textRefs.current[i] = el)}>
                {potion.label}
              </span>
            </button>
          ))}
...
/* Componente isolado para não poluir o HTML principal com os códigos SVG extensos */
const SVGSprite = () => (
  <svg xmlns="http://www.w3.org/2000/svg" style={{ width: 0, height: 0, visibility: 'hidden', position: 'absolute' }} aria-hidden="true">
    <symbol id="potion" viewBox="0 0 18 30">
      <g>
        ...
        <path d="M2.74 23.5c-.23 0-.43-.16-.49-.38C2.08 22.43 2 21.72 2 21c0-.35.02-.71.06-1.06.03-.27.28-.47.55-.44.27.03.47.28.44.55-.03.31-.05.63-.05.94 0 .63.07 1.27.22 1.88.06.27-.1.54-.37.6-.04 0-.08.01-.12.01ZM5 26.5a.46.46 0 0 1-.33-.13c-.37-.32-.71-.68-1.01-1.06-.17-.22-.14-.53.08-.7s.53-.14.7.08c.27.34.57.65.9.94.21.18.23.5.04.71a.48.48 0 0 1-.37.17Z" />
      </g>
    ...
    <symbol id="enhancement" viewBox="0 0 19 30.11">
      <g>
        ...
        <path d="M2.65 25.61c-.27 0-.49-.22-.49-.5 0-.07.01-.14.04-.2l.65-1.5c.11-.25.4-.37.65-.25.25.11.36.41.25.66l-.65 1.5c-.08.18-.25.3-.45.3ZM4.17 22.11c-.27 0-.49-.22-.49-.5 0-.07.01-.14.04-.2l.43-1c.11-.25.4-.37.65-.25.25.11.36.41.25.66l-.43 1c-.08.18-.25.3-.45.3Z" />
      </g>
